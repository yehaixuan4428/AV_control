...
%1.1
ub = [repmat([max(data.bl(1,1), data.br(1,1));max(data.bl(2,9), data.br(2,9));pi], nsteps, 1); repmat([1;0.5], nsteps-1, 1)];
lb = [repmat([min(data.bl(1,9), data.br(1,9));min(data.bl(2,1), data.br(2,1)) ;-pi], nsteps, 1); repmat([0;-0.5], nsteps-1, 1)];
...
xList = linspace(data.cline(1,1), data.cline(1,9), nsteps);
yList = interp1(data.cline(1,1:9), data.cline(2,1:9), xList);
thetaList = interp1(data.cline(1,1:9), data.theta(1:9), xList);
%x0(3*i) = interp1(data.cline(1, 1:2), data.theta(1:2), x0(3*i -2));
%x0(1:2) = data.cline(:, 1);
...
for i = 1:nsteps
    ...
    b(2*i - 1) = - f_bl(1,2) + f_bl(1,1)*data.bl(1,1);
    A(2*i, xIndex(i):yIndex(i)) = [-f_br(1,1), 1.0];
    b(2*i) =  - f_br(1,1)*data.br(1,1) + f_br(1,2);
end
...
function [g,h,dg,dh]=nonlcon(z)
    ...
    %for i = 1:nsteps
        ...
        %index = find(r > R);
        %g(2*i-1) = sign(f_bl(index, 1))*(f_bl(index, 1)*z(xIndex(i)) + f_bl(index, 2) - z(yIndex(i)));
        %g(2*i) = sign(f_br(index, 1))*(z(yIndex(i)) - f_br(index, 1)*z(xIndex(i)) - f_br(index, 2));
        %g(2*i-1) =  z(yIndex(i)) - (f_bl(index, 1)*z(xIndex(i)) + f_bl(index, 2));
        %g(2*i) =  (f_br(index, 1)*z(xIndex(i)) + f_br(index, 2)) - z(yIndex(i));
        %dg(xIndex(i), 2*i - 1) = -f_bl(index,1);
        %dg(yIndex(i), 2*i - 1) = 1.0;
        %dg(xIndex(i), 2*i) = f_br(index,1);
        %dg(yIndex(i), 2*i) = -1.0;
    ...
    h(1:3) = h(1:3) - [data.cline(1,1); data.cline(2,1); data.theta(1)];
...
function [J, dJ] = costfun(z)
    ...
    J = sum((z(xIndex) - endPoint(1)).^2 + (z(yIndex) - endPoint(2)).^2 + (z(psiIndex) - data.theta(1)).^2) + sum(z(deltaIndex).^2);
    %J = sum((z(xIndex) - endPoint(1)).^2 + (z(yIndex) - endPoint(2)).^2);
    dJ = 2.0*z';
    dJ(xIndex) = dJ(xIndex) - endPoint(1)*2;
    dJ(yIndex) = dJ(yIndex) - endPoint(2)*2;
    dJ(psiIndex) = dJ(yIndex) -data.theta(1)*2;
    dJ(uIndex) = 0.0;
    %dJ(deltaIndex) = 0.0;
end
